...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    if current_vocab_size > checkpoint_vocab_size:
        print("Extending the embedding layer to match new vocabulary size")
        # Retrieve old weights and create new ones on CUDA
        ...
        new_embed_weights = torch.cat(
            [old_embed_weights.to("cuda"),  # Ensure the old weights are on CUDA
             torch.zeros(current_vocab_size - checkpoint_vocab_size, old_embed_weights.size(1), device="cuda")],
            dim=0
        )

        # Update the state dictionary with resized weights
...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            self.log_local(pl_module.logger.output_dir, split, images,
