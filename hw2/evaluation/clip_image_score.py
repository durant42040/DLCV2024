...
def calculate_clip_scores(
    input_images: List[Tuple[str, Image.Image]],
        ...
        with torch.no_grad():
            ...
            for _, ref_img in reference_images:
                ref_inputs = processor(
                    images=[ref_img], return_tensors="pt", padding=True
                )
                ref_features = model.get_image_features(**ref_inputs)

                # Calculate similarity scores
                similarity = 100 * torch.nn.functional.cosine_similarity(
...
def calculate_clip_image_scores_folder(
    folder_path: str, reference_folder: str
) -> float:
    ...
    if len(input_images) != 25:
        raise ValueError(
            f"The number of images in the folder must be exactly 25. Found {len(input_images)} images."
        )
