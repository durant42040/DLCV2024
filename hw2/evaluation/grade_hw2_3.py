...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="Directory of the hw2_data/textual_inversion",
    )
    parser.add_argument(
        "--output_dir", type=str, default=".", help="Directory of saved output"
    )
    ...
    for key, value in data.items():
        ...
        for idx, clip_eval in enumerate(value["prompt_4_clip_eval"]):
            ...
            image_scores = calculate_clip_image_scores_folder(
                prompt_output_folder_path, input_folder_path
            )
            text_scores = calculate_clip_text_scores_folder(
                prompt_output_folder_path, clip_eval
            )

            # total_score = image_scores + 2.5 * text_scores
            total_score = [i + 2.5 * j for i, j in zip(image_scores, text_scores)]
            sorted_indices = sorted(
                range(len(total_score)), key=lambda k: total_score[k]
            )[-5:]
            image_score = sum(image_scores[i] for i in sorted_indices) / len(
                sorted_indices
            )
            text_scores = sum(text_scores[i] for i in sorted_indices) / len(
                sorted_indices
            )
