...
class CODALMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'image': image,
            'id': item['id'],
            'answer': item['conversations'][1]['value'] if 'conversations' in item else item.get('answer', ''),
            'question': item['conversations'][0]['value'] if 'conversations' in item else item.get('question', '')
        }
...
def prepare_retrievers(
    ...
    depth_model = DepthAnything.from_pretrained('LiheYoung/depth_anything_vitl14').to(device)
    depth_model.eval()
...
def main():
    parser = argparse.ArgumentParser(description='Build retrieval indexes for downloaded CODA-LM dataset')
