...
class StringStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        ...
        for i, decoded_generation in enumerate(decoded_generations):
            sequence_complete = any(
                stop_string in decoded_generation for stop_string in self.stop_strings
            )
...
class TextEnvironment:
    ...
    def __init__(
        ...
        self.current_device = extract_model_from_parallel(
            self.model
    ...
    def run(self, queries, **rewards_kwargs):
        ...
        histories = [
            TextHistory(q, qt, system=True) for q, qt in zip(queries, queries_tokens)
        ]

        while (
            any(not history.completed for history in histories)
        ...
        queries, responses, masks = map(
            list, zip(*[history.split_query_response_tokens() for history in histories])
        )
    ...
    def parse_tool_call(self, text):
        ...
        """
        result = re.search(
            f"(?<={self.request_token}).*?(?={self.call_token})", text, re.DOTALL
        )
    ...
    def compute_reward(self, histories, **reward_kwargs):
        ...
        rewards = self.reward_fn(
            [history.last_text_segment for history in histories], **reward_kwargs
        )
    ...
    def generate(self, histories):
        ...
        """
        active_histories = [
            i for i, history in enumerate(histories) if not history.completed
        ]
        ...
        for i, response_text, response_tensor in zip(
            active_histories, response_texts, response_tensors
        ):
    ...
    def task_end_check(self, history, model_turn=True):
        ...
        if (
            ...
            and len(self.tokenizer(history.text).input_ids[0]) > self.max_length
        ):
            truncated = True
            ended = True
            ...
            (
                self.request_token in history.last_text_segment
                and self.call_token in history.last_text_segment
            )
    ...
    def _generate_batched(
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            generations = extract_model_from_parallel(self.model).generate(
                **padded_inputs, **self.generation_kwargs
            )
