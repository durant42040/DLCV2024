...
class MergeConfig:
    ...
    def create_merge_config_linear(self) -> "MergeConfiguration":
        ...
        merge_config_dict = {
            ...
            "models": [
                {
                    "model": self.policy_model_path,
                    "parameters": {"weight": self.policy_model_weight},
                },
                {
                    "model": self.target_model_path,
                    "parameters": {"weight": self.target_model_weight},
                },
            ],
        }
    ...
    def create_merge_config_slerp(self) -> "MergeConfiguration":
        ...
        merge_config_dict = {
            ...
            "parameters": {"t": self.t_values},  # Set the t values for SLERP
            "base_model": {
                "model": {"path": self.policy_model_path, "revision": None},
                "lora": None,
                "override_architecture": None,
...
def merge_models(config: MergeConfig, out_path: str):
    ...
    if not is_mergekit_available():
        raise ImportError(
            "merge_models requires the `mergekit` extra. To install, run `pip install trl[mergekit]`."
        )
    run_merge(
        config,
        out_path=out_path,
