...
class XPOTrainer(OnlineDPOTrainer):
    ...
    def _process_completions(self, model_output, ref_output, prompts):
        ...
        model_data = {
            ...
            "attention_mask": torch.cat(
                (prompts["attention_mask"], model_completion_mask), dim=1
            ),
            "raw": prompts["raw"],
        }
        ...
        ref_data = {
            ...
            "attention_mask": torch.cat(
                (prompts["attention_mask"], ref_completion_mask), dim=1
            ),
            "raw": prompts["raw"],
        }
    ...
    def _compute_rewards(self, model_data, ref_data, context_length):
        ...
        if self.args.missing_eos_penalty is not None:
            model_contain_eos = torch.any(
                model_data["input_ids"] == self.processing_class.eos_token_id, dim=-1
            )
            ref_contain_eos = torch.any(
                ref_data["input_ids"] == self.processing_class.eos_token_id, dim=-1
            )
    ...
    def _compute_judge(self, model_data, ref_data, context_length):
        ...
        model_data_completions = [
            completion.strip() for completion in model_data_completions
        ]
        ...
        ref_data_completions = [
            completion.strip() for completion in ref_data_completions
        ]
        ...
        if is_conversational({"prompt": prompts[0]}):
            ...
            model_data_completions = [
                template.render(messages=completion)
                for completion in model_data_completions
            ]

            ref_data_completions = [
                [{"role": "assistant", "content": completion}]
                for completion in ref_data_completions
            ]
            ref_data_completions = [
                template.render(messages=completion)
                for completion in ref_data_completions
        ...
        return torch.tensor(
            [rank == 0 for rank in ranks_of_first_completion],
            device=model_data["input_ids"].device,
        )
    ...
    def _compute_logprobs(self, model, model_data, ref_data, context_length):
        ...
        with torch.no_grad():
            if self.ref_model is None:
                ...
                ref_logprobs_ref_data = compute_logprobs_for_data(
                    self.ref_model, ref_data
                )
        ...
        model_logprobs_ref_data = model_logprobs_ref_data.masked_fill(
            ref_padding_mask, 0.0
        )
    ...
    def _compute_losses(
        ...
        chosen_model_logprobs = torch.where(
            chosen_mask, model_logprobs_model_data_sum, model_logprobs_ref_data_sum
        )
        chosen_ref_logprobs = torch.where(
            chosen_mask, ref_logprobs_model_data_sum, ref_logprobs_ref_data_sum
        )
        chosen_log_ratios = chosen_model_logprobs - chosen_ref_logprobs

        rejected_model_logprobs = torch.where(
            ~chosen_mask, model_logprobs_model_data_sum, model_logprobs_ref_data_sum
        )
        rejected_ref_logprobs = torch.where(
            ~chosen_mask, ref_logprobs_model_data_sum, ref_logprobs_ref_data_sum
        )
    ...
    def _log_statistics(
        ...
        if self.reward_model is not None:
            ...
            self.stats["objective/scores_margin"].append(
                gather_mean(model_scores - ref_scores)
            )
        ...
        self.stats["logps/chosen"].append(
            gather_mean(chosen_model_logprobs.mean() + chosen_ref_logprobs.mean())
        )
        self.stats["logps/rejected"].append(
            gather_mean(rejected_model_logprobs.mean() + rejected_ref_logprobs.mean())
        )
        ...
        ref_eos = (
            ref_data["input_ids"][:, context_length:]
            == self.processing_class.eos_token_id
        ).any(dim=1)
    ...
    def training_step(
        ...
        # Process model completions
        model_data, ref_data = self._process_completions(
            model_output, ref_output, prompts
        )

        # Compute rewards
        if self.reward_model is not None:
            model_scores, ref_scores = self._compute_rewards(
                model_data, ref_data, context_length
            )
