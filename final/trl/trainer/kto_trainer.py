...
def _get_kl_dataset(batch: dict[str, list[Any]]) -> dict[str, list[Any]]:
    ...
    """
    batch["answer_input_ids"] = [batch["answer_input_ids"][-1]] + batch[
        "answer_input_ids"
    ][:-1]
    batch["answer_attention_mask"] = [batch["answer_attention_mask"][-1]] + batch[
...
def _process_tokens(
    ...
    if not kwargs["is_encoder_decoder"]:
        ...
        if (
            ...
            and bos_token_id != all_tokens["prompt_input_ids"][0]
        ):
            max_length -= 1
        if (
            len(all_tokens["answer_input_ids"]) > 0
            and eos_token_id != all_tokens["answer_input_ids"][-1]
        ):
            max_length -= 1
...
class KTOTrainer(Trainer):
    ...
    def prediction_step(
        ...
        logits = torch.tensor(
            [v for k, v in logits_dict.items() if k not in ignore_keys],
    ...
    def evaluation_loop(
        ...
        if self.generate_during_eval:
            ...
            target_indicies = [
                i
                for i in range(len(random_batch["label"]))
                if random_batch["label"][i] is False
            ]
            target_batch = {
