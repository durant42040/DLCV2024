...
class NashMDTrainer(OnlineDPOTrainer):
    ...
    def mixture_coef(self):
        if isinstance(self._mixture_coef, list):
            ...
            return (
                self._mixture_coef[epoch]
                if epoch < len(self._mixture_coef)
                else self._mixture_coef[-1]
            )
        else:
    ...
    def _process_completions(self, model_output, mixture_output, prompts):
        ...
        mixture_completion_ids, mixture_completion_mask = truncate_right(
            mixture_completion_ids,
            self.processing_class.eos_token_id,
            self.processing_class.pad_token_id,
        )
        mixture_data = {
            "input_ids": torch.cat(
                (prompts["input_ids"], mixture_completion_ids), dim=1
            ),
            "attention_mask": torch.cat(
                (prompts["attention_mask"], mixture_completion_mask), dim=1
            ),
            "raw": prompts["raw"],
        }
    ...
    def _compute_rewards(self, model_data, mixture_data, context_length):
        ...
        if self.args.missing_eos_penalty is not None:
            ...
            mixture_contain_eos = torch.any(
                mixture_data["input_ids"] == self.processing_class.eos_token_id, dim=-1
            )
    ...
    def _compute_judge(self, model_data, mixture_data, context_length):
        ...
        mixture_data_completions = [
            completion.strip() for completion in mixture_data_completions
        ]
    ...
    def training_step(
        ...
        # Process model completions
        model_data, mixture_data = self._process_completions(
            model_output, mixture_output, prompts
        )

        # Compute rewards
        if self.reward_model is not None:
            model_scores, mixture_scores = self._compute_rewards(
                model_data, mixture_data, context_length
            )
        ...
        # Compute logprobs
        model_logprobs_model_data, ref_logprobs_model_data = self._compute_logprobs(
            model, model_data, context_length
        )
