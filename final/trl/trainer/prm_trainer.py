...
class PRMTrainer(Trainer):
    ...
    def __init__(
        ...
        if data_collator is None:
            ...
            data_collator = DataCollatorForTokenClassification(
                processing_class, max_length=args.max_length
            )
    ...
    def tokenize_row(
        ...
        prompt_ids = tokenizer(features["prompt"], add_special_tokens=False)[
            "input_ids"
        ]
        completions_ids = [
        ...
        if train_on_last_step_only and not is_eval:
            labels = [-100] * (len(features["labels"]) - 1) + [
                int(features["labels"][-1])
            ]
        else:
