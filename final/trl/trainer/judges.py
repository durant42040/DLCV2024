    ...
    **Example**:
    ...
    def judge(
        ...
        ranks = self.blender.rank(
            prompts, completions, return_scores=return_scores, disable_tqdm=True
        )
        if not return_scores:
            ranks -= (
                1  # PairRM rank is 1-indexed, so we subtract 1 to make it 0-indexed
            )
        else:
        ...
        if return_scores:
            ...
            probs = exp_logit_shifted / np.sum(
                exp_logit_shifted, axis=-1, keepdims=True
            )
    ...
    def judge(
        ...
        def get_rank(prompt, candidates):
            ...
            completion = self.client.chat_completion(
                messages=[{"role": "user", "content": content}], max_tokens=1
            )
...
class AllTrueJudge(BaseBinaryJudge):
    ...
    def judge(
        ...
        all_binary_judgments = [
            judge.judge(prompts, completions, gold_completions, shuffle_order)
        ...
        for binary_judgments in zip(*all_binary_judgments):
            ...
            if any(
                binary_judgment not in {0, 1, -1}
                for binary_judgment in binary_judgments
            ):
                raise ValueError(
