...
class WinRateCallback(TrainerCallback):
    ...
    def __init__(
        ...
        if self.trainer.eval_dataset is None:
            raise ValueError(
                "Trainer must have an evaluation dataset to use the WinRateCallback."
            )
        else:
        ...
        **kwargs,
        ...
        with accelerator.split_between_processes(
            ...
            if self.use_soft_judge:
                ref_win_probs = self.judge.judge(
                    prompts, completions, self.shuffle_order, return_scores=True
                )
                ...
                winner_indices = self.judge.judge(
                    prompts, completions, self.shuffle_order
                )
        ...
        if self.trainer.accelerator.is_main_process:
            ...
            if self.use_soft_judge:
                ...
                self.trainer.log(
                    {"eval_avg_win_prob": avg_win_prob, "eval_win_rate": win_rate}
                )
            else:
...
class LogCompletionsCallback(WandbCallback):
    ...
    def __init__(
        ...
        if self.trainer.eval_dataset is None:
            raise ValueError(
                "Trainer must have an evaluation dataset to use the LogCompletionsCallback."
            )
        else:
            self.eval_dataset = self.trainer.eval_dataset
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if self.trainer.accelerator.is_main_process:
            ...
            table = self._wandb.Table(
                columns=["step", "prompt", "completion"], data=self.table
            )
