...
def init_zero_verbose():
    ...
    FORMAT = "%(message)s"
    logging.basicConfig(
        format=FORMAT, datefmt="[%X]", handlers=[RichHandler()], level=logging.ERROR
    )
...
class TrlParser(HfArgumentParser):
    ...
    def parse_args_and_config(
        ...
        output = self.parse_args_into_dataclasses(
            args=args, return_remaining_strings=return_remaining_strings
        )
    ...
    def set_defaults_with_config(self, **kwargs) -> list[str]:
        ...
        remaining_strings = [
            item for key, value in kwargs.items() for item in [f"--{key}", str(value)]
        ]
...
def get_git_commit_hash(package_name):
    try:
        ...
        if os.path.isdir(git_dir):
            ...
            commit_hash = (
                subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=git_repo_path)
                .strip()
                .decode("utf-8")
            )
            return commit_hash
