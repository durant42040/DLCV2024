...
def pipeline_step_with_grad(
    ...
    with pipeline.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if truncated_backprop:
                ...
                if truncated_backprop_rand:
                    rand_timestep = random.randint(
                        truncated_rand_backprop_minmax[0],
                        truncated_rand_backprop_minmax[1],
                    )
                    if i < rand_timestep:
            ...
            # compute the previous noisy sample x_t -> x_t-1
            scheduler_output = scheduler_step(
                pipeline.scheduler, noise_pred, t, latents, eta
            )
            latents = scheduler_output.latents
            log_prob = scheduler_output.log_probs
    ...
    # Offload last model to CPU
    if (
        hasattr(pipeline, "final_offload_hook")
        and pipeline.final_offload_hook is not None
    ):
...
class DefaultDDPOStableDiffusionPipeline(DDPOStableDiffusionPipeline):
    ...
    def save_pretrained(self, output_dir):
        if self.use_lora:
            state_dict = convert_state_dict_to_diffusers(
                get_peft_model_state_dict(self.sd_pipeline.unet)
            )
            self.sd_pipeline.save_lora_weights(
                save_directory=output_dir, unet_lora_layers=state_dict
            )
    ...
    def save_checkpoint(self, models, weights, output_dir):
        if len(models) != 1:
            raise ValueError(
                "Given how the trainable params were set, this should be of length 1"
            )
        if (
            self.use_lora
            and hasattr(models[0], "peft_config")
            and getattr(models[0], "peft_config", None) is not None
        ):
            state_dict = convert_state_dict_to_diffusers(
    ...
    def load_checkpoint(self, models, input_dir):
        ...
        if self.use_lora:
            ...
            self.sd_pipeline.load_lora_into_unet(
                lora_state_dict, network_alphas=network_alphas, unet=models[0]
            )
