...
class PreTrainedModelWrapper(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if isinstance(pretrained_model_name_or_path, str):
            ...
            load_kwargs = (
                {} if use_safe else {"map_location": "cpu", "weights_only": True}
            )
    ...
    def add_and_load_reward_modeling_adapter(
        ...
        load_kwargs = (
            {} if safe_loading else {"map_location": "cpu", "weights_only": True}
        )
...
class GeometricMixtureWrapper(GenerationMixin):
    ...
    def __init__(
        self, model, ref_model, generation_config, mixture_coef=0.5, device=None
    ):
        super().__init__()
    ...
    def forward(self, *args, **kwargs):
        ...
        model_outputs.logits = torch.nn.functional.log_softmax(
            self.mixture_coef * ref_model_logits
            + (1 - self.mixture_coef) * model_logits,
            dim=-1,
        )
