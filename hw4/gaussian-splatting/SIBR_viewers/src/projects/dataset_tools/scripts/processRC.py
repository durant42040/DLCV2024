...
from utils.commands import getColmap, getProcess, getRCprocess
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="path to rc dataset, containing bundle.out and images",
    )
    ...
    parser.add_argument(
        "--create_colmap", action="store_true", help="create colmap hierarchy"
    )
    parser.add_argument(
        "--target_width", type=str, default="default", help="colmap_target_width"
    )
    parser.add_argument(
        "--from_step",
        type=str,
        default="default",
        help="Run from this step to --to_step (or end if no to_step",
    )
    parser.add_argument(
        "--to_step",
        type=str,
    ...
    parser.add_argument(
        "--car_data", action="store_true", help="pre(pre)process car camera data "
    )
    parser.add_argument("--do_train", action="store_false", help="use train folder")
    parser.add_argument(
        "--do_validation", action="store_false", help="use validation folder"
    )
    parser.add_argument(
        "--no_validation_split", action="store_true", help="dont do validation split"
    )
    parser.add_argument(
        "--do_video", action="store_true", help="use video folder (mp4)"
    )
    parser.add_argument(
        "--do_test", action="store_true", help="use test folder (stills path)"
    )
    parser.add_argument(
        "--auto_recon_area",
        action="store_true",
        help="automatically set recon area (no user intervention)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        help="folder containing configuration files; usually cwd",
    )
    parser.add_argument(
        "--model_name", type=str, default="default", help="Internal name of RC model"
    )
    parser.add_argument(
        "--path_prefix",
        type=str,
        default="default",
        help="Internal prefix of path images",
    )
    parser.add_argument(
        "--one_over_fps",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        help="skip every nth image for validation",
    )
    # "presets"
    parser.add_argument(
        "--images_only",
        action="store_false",
        help="just process images: no validation, no test",
    )
    parser.add_argument(
        "--video_only",
        action="store_true",
        help="just process video: no photos, no test",
    )

    parser.add_argument(
        "--no_refl",
        action="store_true",
        help="dont densify mesh, dont convert_sibr, dont create nerf (def: false)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        help="full path of video file (internal argument; do not set)",
    )
    parser.add_argument(
        "--mesh_obj_filename",
        type=str,
        default="default",
        help="full path of obj mesh file (internal argument; do not set)",
    )
    parser.add_argument(
        "--mesh_xyz_filename",
        type=str,
        default="default",
        help="full path of xyz point cloud file (internal argument; do not set)",
    )
    parser.add_argument(
        "--mesh_ply_filename",
        type=str,
        default="default",
        help="full path of ply mesh file (internal argument; do not set)",
    )
    ...
    args["mesh_obj_filename"] = os.path.join(
        args["path"], os.path.join("rcScene", os.path.join("meshes", "mesh.obj"))
    )
    args["mesh_xyz_filename"] = os.path.join(
        args["path"], os.path.join("rcScene", os.path.join("meshes", "point_cloud.xyz"))
    )
    args["mesh_ply_filename"] = os.path.join(
        args["path"], os.path.join("sibr", os.path.join("capreal", "mesh.ply"))
    )

    args["path_prefix"] = "test_"

    # fixed in preprocess
    args["video_filename"] = os.path.join(
        args["path"], os.path.join("raw", os.path.join("videos", "XXX.mp4"))
    )
