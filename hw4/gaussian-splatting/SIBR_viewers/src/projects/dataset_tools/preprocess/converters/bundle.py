...
class BundleCamera:
    ...
    def __str__(self):
        first_line = "{0:g} {1:g} {2:g}\n".format(
            self.focal_length, self.radial_dist[0], self.radial_dist[1]
        )
...
class BundleFeaturePoint:
    ...
    def __str__(self):
        ...
        for view_info in self.view_list:
            third_line = third_line + "{0:g} {1:g} {2:g} {3:g}".format(
                view_info[0], view_info[1], view_info[2], view_info[3]
            )
...
class Bundle:
    ...
    def __init__(self, path_to_bundle):
        ...
        self.nr_cameras, self.nr_feature_points = map(
            int, input_file.readline().strip().split(" ")
        )

        self.using_feature_points = (
            self.nr_feature_points < Bundle.MAX_NR_FEATURE_POINTS
        )
        if not self.using_feature_points:
            self.nr_feature_points = 0
            print(
                "[bundle.py] Warning: Too many feature points. They are going to be discarded"
            )
        ...
        for i in range(self.nr_cameras):
            # read each camera
            focal_length, radial_dist_x, radial_dist_y = map(
                float, input_file.readline().strip().split(" ")
            )
        ...
        if self.using_feature_points:
            ...
            for line in input_file:
                if type_of_line_to_read == BundleFeaturePointLine.POSITION:
                    ...
                    feature_point = BundleFeaturePoint(
                        feature_point_id,
                        feature_point_position,
                        feature_point_color,
                        list_of_view_info,
                    )
                    ...
                    for v in list_of_view_info:
                        if v[0] >= len(self.list_of_cameras):
                            ...
                            self.list_of_cameras[v[0]].list_of_feature_points.append(
                                feature_point
                            )
                ...
                type_of_line_to_read = (type_of_line_to_read + 1) % len(
                    BundleFeaturePointLine
                )
        ...
        for file_in_dir in os.listdir(self.root_directory):
            ...
            if (
                file_in_dir.lower().endswith(".jpg")
                or file_in_dir.lower().endswith(".png")
                or file_in_dir.lower().endswith(".jpeg")
            ):
        ...
        self.has_right_nr_images = len(self.list_of_cameras) == len(
            self.list_of_input_images
        )

        if not self.has_right_nr_images:
            print(
                "[bundle.py] Warning: nr cameras in bundle file ("
                + str(len(self.list_of_cameras))
                + ") is not the same as nr of images in "
                + self.root_directory
                + " ("
                + str(len(self.list_of_input_images))
                + ")"
            )
        ...
        print(
            "[bundle.py] Message: Nr images in root folder",
            len(self.list_of_input_images),
        )
        print(
            "[bundle.py] Message: Nr feature points", len(self.list_of_feature_points)
        )
    ...
    def save(self, path_to_output_file, new_res=[]):
        ...
        output_file.write(
            str(self.nr_cameras) + " " + str(self.nr_feature_points) + "\n"
        )
