...
def simplifyMesh(inputMesh, outputMesh, meshsize="", meshlabPath=getMeshlabPath()):
    ...
    mlxFile = os.path.abspath(
        os.path.join(os.path.abspath(os.path.dirname(__file__)), mlxFileEnd)
    )

    return runCommand(
        getMeshlabServer(meshlabPath),
        ["-i", inputMesh, "-o", outputMesh, "-s", mlxFile],
    )
...
def main():
    ...
    # common arguments
    parser.add_argument(
        "--inputMesh", type=str, required=True, help="the mesh to simplify"
    )
    ...
    return simplifyMesh(
        args["inputMesh"], args["outputMesh"], args["meshsize"], args["meshlabPath"]
    )
