...
def main(argv, path_dest):
    ...
    for opt, arg in opts:
        if opt == "-h":
            print(
                "-i path_to_rc_data_dir -o path_to_destination_dir [-r (use release w/ debug symbols executables)]"
            )
            sys.exit()
        elif opt == "-i":
            path_data = arg
...
path_dest = ""
path_data, path_dest, executables_suffix, executables_folder = main(
    sys.argv[1:], path_dest
)
...
crop_from_center_app = getProcess(
    "cropFromCenter" + executables_suffix, executables_folder
)
...
crop_from_center_args = [
    crop_from_center_app,
    "--inputFile",
...
if target_res is not None:
    ...
    path_to_scaled_down_output_bundle = os.path.join(
        os.path.join(path_dest, "images/scaled"), "bundle.out"
    )
...
clipping_planes_app = getProcess(
    "clippingPlanes" + executables_suffix, executables_folder
)
...
if os.path.exists(os.path.join(path_dest, pmvs_model_dir, "pmvs_recon.ply")):
    shutil.copy(
        os.path.join(path_dest, pmvs_model_dir, "pmvs_recon.ply"),
        os.path.join(path_dest, pmvs_model_dir, "recon.ply"),
    )

if os.path.exists(os.path.join(path_dest, pmvs_model_dir, "mesh.ply")):
    shutil.copy(
        os.path.join(path_dest, pmvs_model_dir, "mesh.ply"),
        os.path.join(path_dest, pmvs_model_dir, "recon.ply"),
    )
