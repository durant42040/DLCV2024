...
def main():
    ...
    # common arguments
    parser.add_argument(
        "--path", type=str, required=True, help="path to dataset folder"
    )
    ...
    if not checkColmapConsistent(args["path"]):
        print(
            "SIBR_ERROR Colmap hasnt been run properly; run it first (ie dont use --noColmap)"
        )
        sys.exit(1)
