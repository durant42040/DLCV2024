...
def main():
    ...
    # common arguments
    parser.add_argument(
        "--imagesPath", type=str, required=True, help="path to your images folder"
    )
    parser.add_argument(
        "--outputPath",
        type=str,
        default=None,
        help="output path where to place the list_images.txt",
    )
    parser.add_argument(
        "--filename", type=str, default=None, help="filename for the image list"
    )
    ...
    if not args["outputPath"]:
        ...
        not os.path.isdir(args["outputPath"])
        and os.path.basename(args["outputPath"])
    ...
    print(
        "Generating '%s' file from images in '%s' and saving to '%s'."
        % (args["filename"], args["imagesPath"], args["outputPath"])
    )

    generateListImages(
        os.path.abspath(args["imagesPath"]),
        os.path.abspath(args["outputPath"]),
        args["filename"],
    )
