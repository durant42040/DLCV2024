...
def extract_images(pathIn, pathOut, videoName, maxNumFrames=-1, resize=False):
    ...
    for frame in range(round(total_frames)):
        ...
        print(
            ...
            EVERY_NTH * frame,
            " ",
            pathOut + "\\%s\\frame%04d.png" % (videoName, count),
        )
        fileNames.append("%s\\frame%04d.png" % (videoName, count))
        cv2.imwrite(
            pathOut + "\\%s\\frame%04d.png" % (videoName, count), resized
...
def remove_video_images(path, photoName="MG_"):
    ...
    images_fname = (
        os.path.abspath(os.path.join(path, "colmap\\sparse\\")) + "\\images.txt"
    )
    ...
    cameras_fname = (
        os.path.abspath(os.path.join(path, "colmap\\sparse\\")) + "\\cameras.txt"
    )
    ...
    new_images_data = []
    new_images_data, img_cnt = extract_images_with_name(
        photoName, images_data, new_images_data
    )
    ...
    new_images_data[3] = (
        " ".join(images_data[3].split()[0:4])
        + " "
        + str(img_cnt)
        + " "
        + " ".join(images_data[3].split()[5:-1])
    )
    ...
    for currVideoName in videoDirList:
        ...
        imagespath = os.path.abspath(os.path.join(path, "images"))
        shutil.move(
            imagespath + "\\" + currVideoName, backuppath + "\\" + currVideoName
        )
        ...
        video_images_list = []
        video_images_list, img_cnt = extract_images_with_name(
            currVideoName, images_data, video_images_list
        )
        video_images_list[2] = (
            " ".join(images_data[2].split()[0:4])
            + " "
            + str(img_cnt)
            + " "
            + " ".join(images_data[2].split()[5:-1])
        )
        ...
        points_fname = (
            os.path.abspath(os.path.join(path, "colmap\\sparse\\")) + "\\points3D.txt"
        )
        shutil.copyfile(points_fname, dstpath + "\\points3D.txt")

        fname = os.path.abspath(
            os.path.join(path, "colmap\\stereo\\stereo\\fusion.cfg")
        )
        remove_lines_from_file(fname, "Video", False)
        fname = os.path.abspath(
            os.path.join(path, "colmap\\stereo\\stereo\\patch-match.cfg")
        )
...
def fix_cameras(path, photoName="MG_", sparseSubdir=""):
    ...
    if os.path.exists(oldb):  # only do DB processing if it exists
        ...
        delCamQuery = (
            """DELETE from cameras WHERE camera_id != '%s' and camera_id != '%s'"""
            % (videoCamIndex, photoCamIndex)
        )
        cursor.execute(delCamQuery)

        # change photo cam id to 1 and video cam id to 2
        setQuery = "UPDATE cameras SET camera_id = '%s' WHERE  camera_id = '%s'" % (
            "1",
            photoCamIndex,
        )
    ...
    for filename in os.listdir(pathIn):
        if ("MP4" in filename) or ("mp4" in filename):
            with open(os.path.join(pathIn, filename), "r") as f:
                ...
                fileNames = fileNames + extract_images(
                    f.name, pathOut, "Video%d" % cnt, resize=True
                )
