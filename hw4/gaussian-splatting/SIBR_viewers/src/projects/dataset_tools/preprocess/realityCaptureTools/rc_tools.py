...
from utils.commands import getColmap, getProcess, getRCprocess, runCommand
from utils.paths import getBinariesPath, getColmapPath, getMeshlabPath


def preprocess_for_rc(
    path, video_name="default", do_validation_split=True, valid_skip="10"
):
    ...
    if not os.path.exists(imagespath):
        ...
        elif not os.path.exists(os.path.join(path, "videos")) and not os.path.exists(
            videopath
        ):
            print(
                "ERROR: No images nor video, exiting. Images should be in $path/raw/images"
            )
            exit(-1)
        # videos are optional
    ...
    video_filename = os.path.join(
        path, os.path.join("raw", os.path.join("videos", video_name))
    )
...
def convert_sibr_mesh(path):
    ...
    mesh_path = os.path.join(
        os.path.join(os.path.join(path, "rcScene"), "meshes"), "mesh.obj"
    )
    print("Loading mesh (slow...)", mesh_path)
    ms.load_new_mesh(mesh_path)
    meshply_path = out_mesh_path = os.path.join(
        os.path.join(os.path.join(path, "sibr"), "capreal"), "mesh.ply"
    )
    ...
    texture_path = os.path.join(
        os.path.join(os.path.join(path, "sibr"), "capreal"), "mesh_u1_v1.png"
    )
    out_texture_path = os.path.join(
        os.path.join(os.path.join(path, "sibr"), "capreal"), "texture.png"
    )
    print(
        "Copying (to allow meshlab to work) {} to {}".format(
            texture_path, out_texture_path
        )
    )
    shutil.copyfile(texture_path, out_texture_path)
    out_mesh_path = os.path.join(
        os.path.join(os.path.join(os.path.join(path, "sibr"), "colmap"), "stereo"),
        "meshed-delaunay.ply",
    )
                ...
                for v in p.view_list:
                    if v[0] == camera_id - 1:
                        outfile.write(
                            ...
                            + str(scale * (2.0 * v[3] + h))
                            + " -1"
                        )  # TODO: not sure about this, seems to be -1 in all existing files
                        if not first:
                            outfile.write(" ")
                        else:
                            first = False
...
def crop_images(path_data, path_dest):
    ...
    if "validation_" not in path_data:
        ...
        print(
            "Command: run distordCrop ARGS: ",
            "--path",
            path_data,
            "--ratio",
            "0.3",
            "--avg_width",
            str(avg_resolution[0]),
            "--avg_height",
            str(avg_resolution[1]),
            ")",
        )
        retcode = runCommand(
            getProcess("distordCrop"),
            [
                "--path",
                path_data,
                "--ratio",
        ...
        if retcode.returncode != 0:
            print(
                "Command: distordCrop failed, exiting (ARGS: ",
                "--path",
                path_data,
                "--ratio",
                "0.3",
    ...
    print(
        "Command: run cropFromCenter ARGS:",
        "--inputFile",
    ...
    if retcode.returncode != 0:
        print(
            "Command: cropFromCenter failed, exiting (ARGS:",
            "--inputFile",
            path_to_transform_list_txt,
            "--outputPath",
            path_dest,
...
def car_data_process(path):
    ...
    for dirname in dirlist:
        ...
        for filename in os.listdir(campath):
            shutil.copyfile(
                os.path.join(campath, filename),
                os.path.join(imagespath, "{:06d}".format(global_im_counter) + ".jpg"),
            )
            global_im_counter += 1
